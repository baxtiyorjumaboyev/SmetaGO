...
/* ---------- state ---------- */
function mkRoom(type,name,L,W,H){const t=ROOM_TYPES[type]||ROOM_DEFAULT;return{id:uid(),type,name:name||type,L:L??"",W:W??"",H:H??"2,8",doors:["0,9"],windows:[],floor:t.floor,wall:t.wall,ceil:t.ceil,tileLen:"",tileH:"",plinthOv:"",items:[]}}
...
function blank(base,name){const nr=mkRoom("Mehmonxona");return{v:1,sample:false,obj:{name:name||"Yangi obyekt",region:base.obj.region,quarter:base.obj.quarter},settings:base.settings,prices:base.prices,rooms:[nr],concrete:[],ui:{tab:"xonalar",room:nr.id,grp:"Tavsiya",q:""}}}
...
function persist(){saveT=null;const body=JSON.stringify(S);
  ...
  fetch(SERVER.saveUrl,{method:"PUT",credentials:"same-origin",keepalive:body.length<60000,headers:{"Content-Type":"application/json","X-CSRFToken":SERVER.csrf},body})
   ...
   .catch(e=>toast(e.message==="auth"?"Sessiya tugagan — qayta kiring":"Serverga saqlanmadi, qayta urinib ko'ring"))}
...
function renderCatItems(){
  ...
  else if(S.ui.grp==="Tavsiya"){const t=ROOM_TYPES[r?.type]||ROOM_DEFAULT;html=`<p class="eyebrow" style="margin:0">${esc(r?.type||"")} uchun odatiy</p><div class="catgrid">${t.s.map(id=>CAT_INDEX[id]).filter(Boolean).map(it=>card(it,1)).join("")}</div>`}
  else{const g=CATALOG.find(g=>g.g===S.ui.grp);html=`<div class="catgrid">${(g?g.items:[]).map(it=>card(it)).join("")}</div>`}
  box.innerHTML=html;
}
...
function viewPrices(){
  ...
  return `<div class="pagehead"><div><h2>Narxlar bazasi — ${esc(S.obj.quarter)}</h2><p>Har bir material uchun 3 ta manbadan narx kiriting (zavod, karyer, do'kon, birja). Dastur o'rtachasini hisoblaydi; kerakli narxni tanlang. Narxlar har chorakda yangilanadi.</p></div>
   <div class="row"><button class="btn" data-act="syncPrices" title="Manba narxlari ${SERVER?"markaziy bazadan":"standart qiymatlardan"} olinadi; tanlov (o'rtacha / qo'lda) saqlanadi">${S.ui.confirmSync?"Tasdiqlang: manba narxlari almashadi":SERVER?"Markaziy narxlarni yuklash":"Standart narxlarga qaytarish"}</button>${REF&&REF.pricesUpdated?`<span class="note">Markaziy baza: ${esc(REF.pricesUpdated)}</span>`:""}</div></div>
...
document.addEventListener("click",e=>{const b=e.target.closest("[data-act]");if(!b)return;const a=b.dataset.act;const r=curRoom();
  ...
  switch(a){
   ...
   case "syncPrices":if(!S.ui.confirmSync){S.ui.confirmSync=true;render();break}
    ...
    S.ui.confirmSync=false;render();toast("Manba narxlari yangilandi");break;
